...
namespace EduTrail.Infrastructure.Repositories
{
    public class PostRepository : IPostRepository
    {
        ...
        public async Task<IEnumerable<Enrollment>> GetEnrollmentsByCourseOfferingAsync(Guid courseOfferingId)
        {
            return await _context.Enrollments.Include(c => c.User).Where(c => c.CourseOfferingId == courseOfferingId).ToListAsync();
        }

        public async Task<IEnumerable<Folder>> GetFoldersByCourseOfferingAsync(Guid courseOfferingId)
        {
            return await _context.Folders.Where(c => c.CourseOfferingId == courseOfferingId).ToListAsync();
        }

        public async Task<List<Folder>> GetFoldersByIdsAsync(List<Guid> folderIds)
        {
